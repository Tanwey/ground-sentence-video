            ...
            with open(os.path.join(textual_data_path, file)) as tsvfile:
                ...
                for row in reader:
                    ...
                    self.captions += [Caption(video_id=video_id, start_time=start_frame / self.fps, 
                                              end_time=end_frame / self.fps, sent=sent) for sent in sents]
...
#         print('train set size is %d' % len(self.train_indices), file=sys.stderr)
            ...
            for t in range(T):
                for k in range(self.K):
                    if (compute_overlap((t - (k+1) * self.delta) * self.sample_rate / self.fps,
                                        t * self.sample_rate / self.fps, start_time, end_time)) > self.threshold:
                        label[t, k] = 1
    ...
    def __getitem__(self, item):
        ...
        :returns the features of the corresponding video as a torch.Tensor, the caption and the 
        ground-truth label 
        """
    ...
    def data_iter(self, batch_size: int, set: str):
        """
        :param batch_size
        :param set: specifies the part of the data to iterate over as a str ('train', 'val', or 'test')
...
class ActivityNet(torch.utils.data.Dataset):
    def __init__(self, textual_data_path: str, visual_data_path: str, K: int, delta: int, threshold: float):
        ...
        '''
        the provided C3D features have been extracted every 8 frames. In this code I further sample one 
        frame out of every three frames which leads to one frame in every 24 frames of the original video 
...
#         print('number of train instances', len(self.train_captions))
        ...
        with open(os.path.join(textual_data_path, 'val_1.json')) as json_file:
            ...
            for key, value in data.items():
                ...
                sents = value['sentences']
                sents = [TOKENIZER.tokenize(sent.lower()) for sent in sents]
                self.val_captions += [Caption(video_id=key, start_time=time_stamp[0], end_time=time_stamp[1], 
                                              sent=sent) for time_stamp, sent in zip(time_stamps, sents)]
...
#         print('number of val instances', len(self.val_captions))
    ...
    def _generate_labels(self, visual_data: List[torch.Tensor], captions: List[Caption]):
        ...
        :param textual_data: list of annotations
        :returns labels for the samples as a torch.Tensor with the shape (num_labels, max_time_steps, K)
        """
